...
try {
    ...
    $assetTarget = Join-Path $phase9 'assets'
    if (-not (Test-Path -LiteralPath $assetSource -PathType Container)) { throw 'The interface assets are missing.' }
    New-Item -ItemType Directory -Path $assetTarget -Force | Out-Null
    Copy-Item -LiteralPath (Join-Path $assetSource 'black-gold-marble.png') -Destination $assetTarget -Force
    Copy-Item -LiteralPath (Join-Path $assetSource 'printstream.png') -Destination $assetTarget -Force
